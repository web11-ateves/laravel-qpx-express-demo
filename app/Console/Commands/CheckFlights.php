...
class CheckFlights extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($trips as $trip) {
            ...
            if(!$trip->departure_date_end){
                $trip->departure_date_end = $trip->departure_date->format('d/m/Y');
            }

            $count = 0;
            for($date = $trip->departure_date; $date->lte($trip->departure_date_end); $date->addDay()) {

                $this->line($trip->description . ' @' . $date->format("Y-m-d"));
